...
fn main() {
    ...
    let unused_space = 70_000_000 - directory_sizes.get("/").unwrap();
    const MIN_REQUIRED_SPACE: u32 = 30_000_000;

    let result: &u32 = directory_sizes
        .values()
        .filter(|v| **v + unused_space >= MIN_REQUIRED_SPACE)
        .min()
        .unwrap();
